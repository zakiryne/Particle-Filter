...
void ParticleFilter::dataAssociation(std::vector<LandmarkObs> predicted, std::vector<LandmarkObs>& observations) {
	...
	for (unsigned int i = 0; i < nObservations; i++) { // For each observation
		...
		for (unsigned j = 0; j < nPredictions; j++ ) { // For each prediction.
...
void ParticleFilter::updateWeights(double sensor_range, double std_landmark[], 
	...
	//   http://planning.cs.uiuc.edu/node99.html
	  ...
	  for (int i = 0; i < num_particles; i++) {
	    ...
	    double theta = particles[i].theta;
	    // Find landmarks in particle's range. Only consider landmarks within sensor range of the particle
	    ...
	    // Transform observation coordinates. Create and populate a copy of the list of observations transformed from vehicle coordinates to map coordinates
	    ...
	    // Observation association to landmark. Perform dataAssociation for the predictions and transformed observations on current particle
